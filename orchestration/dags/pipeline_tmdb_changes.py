...
with DAG(
    ...
    ingestar_cambios_api = DockerOperator(
        ...
        command="/bin/bash -c 'pip install -r ingestion/requirements.txt && python ingestion/dlthub_api_consumer.py'",
    ...
    transformar_cambios_db = DockerOperator(
        ...
        command="/bin/bash -c 'pip install -r transform/requirements.txt && cd transform && dbt build'",
        docker_url='unix://var/run/docker.sock',
        network_mode='etl-network',  # Connect to the same network as other services
        mounts=[
            Mount(
